...
async function chat(clientId, phone, userMessage, channel = "sms") {
  ...
  if (client.menuOrServices && client.menuOrServices.length > 0) {
    const available   = client.menuOrServices.filter(i => i.available !== false);
    const unavailable = client.menuOrServices.filter(i => i.available === false);

    const label = client.businessType === "restaurant" ? "MENU" : "SERVICES/PRODUCTS";
    servicesText = `\n\nAVAILABLE ${label} TODAY:\n`;

    if (available.length === 0) {
      servicesText += "  (No items currently available — inform the customer politely)\n";
    } else {
      const cats = [...new Set(available.map(i => i.category))];
      for (const cat of cats) {
        servicesText += `\n${cat.toUpperCase()}\n`;
        available.filter(i => i.category === cat).forEach(i => {
    ...
    if (unavailable.length > 0) {
      servicesText += `\nUNAVAILABLE TODAY (do NOT take orders for these — tell customer they are unavailable and suggest alternatives from the available list):\n`;
